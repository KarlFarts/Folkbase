...
describe('Dev Mode Data Integrity', () => {
  ...
  describe('Migration Utility', () => {
    it('should migrate E001 format to EVT001', async () => {
      localStorage.setItem(
        STORAGE_KEY_EVENTS,
        JSON.stringify([
          { 'Event ID': 'E001', 'Event Name': 'Old Event 1' },
          { 'Event ID': 'E002', 'Event Name': 'Old Event 2' },
        ])
      );

      const { migrateLocalStorageIds } = await import('../migrateLocalStorageIds');
      const results = migrateLocalStorageIds();

      expect(results.eventsFixed).toBe(2);

      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY_EVENTS));
      expect(stored[0]['Event ID']).toBe('EVT001');
      expect(stored[1]['Event ID']).toBe('EVT002');
    });

    it('should migrate TASK001 format to TSK001', async () => {
      localStorage.setItem(
        STORAGE_KEY_TASKS,
        JSON.stringify([
          { 'Task ID': 'TASK001', Title: 'Old Task 1' },
          { 'Task ID': 'TASK002', Title: 'Old Task 2' },
        ])
      );
      ...
      expect(results.tasksFixed).toBe(2);

      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY_TASKS));
      expect(stored[0]['Task ID']).toBe('TSK001');
      expect(stored[1]['Task ID']).toBe('TSK002');
    });

    it('should update event note references during migration', async () => {
      localStorage.setItem(
        STORAGE_KEY_EVENTS,
        JSON.stringify([{ 'Event ID': 'E001', 'Event Name': 'Event' }])
      );
      localStorage.setItem(
        STORAGE_KEY_EVENT_NOTES,
        JSON.stringify([{ 'Event ID': 'E001', 'Note ID': 'NOTE001' }])
      );

      const { migrateLocalStorageIds } = await import('../migrateLocalStorageIds');
      const results = migrateLocalStorageIds();

      expect(results.eventsFixed).toBe(1);
      expect(results.eventNotesFixed).toBe(1);

      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY_EVENT_NOTES));
      expect(stored[0]['Event ID']).toBe('EVT001');
    });

    it('should not modify already-correct IDs', async () => {
      localStorage.setItem(
        STORAGE_KEY_EVENTS,
        JSON.stringify([{ 'Event ID': 'EVT001', 'Event Name': 'Correct Format' }])
      );
      localStorage.setItem(
        STORAGE_KEY_TASKS,
        JSON.stringify([{ 'Task ID': 'TSK001', Title: 'Correct Format' }])
      );

      const { migrateLocalStorageIds } = await import('../migrateLocalStorageIds');
      const results = migrateLocalStorageIds();

      expect(results.eventsFixed).toBe(0);
      expect(results.tasksFixed).toBe(0);
    });

    it('previewMigration should show what would be changed', async () => {
      localStorage.setItem(
        STORAGE_KEY_EVENTS,
        JSON.stringify([{ 'Event ID': 'E003', 'Event Name': 'Old' }])
      );
      localStorage.setItem(
        STORAGE_KEY_TASKS,
        JSON.stringify([{ 'Task ID': 'TASK005', Title: 'Old' }])
      );

      const { previewMigration } = await import('../migrateLocalStorageIds');
      const preview = previewMigration();

      expect(preview.eventsToMigrate).toEqual([{ old: 'E003', new: 'EVT003' }]);
      expect(preview.tasksToMigrate).toEqual([{ old: 'TASK005', new: 'TSK005' }]);
    });
  });
});
